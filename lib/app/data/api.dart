...
import 'package:app_api/app/model/changepassword.dart';
...
class APIRepository {
  ...
  Future<String> updateProfile(
      String token, UpdateProfileM updateProfileM) async {
    print('update profile token:' + token);
    try {
      final body = FormData.fromMap({
        "NumberID": updateProfileM.numberID,
        "FullName": updateProfileM.fullName,
        "PhoneNumber": updateProfileM.phoneNumber,
        "ImageURL": updateProfileM.imageUrl,
        "BirthDay": updateProfileM.birthDay,
        "Gender": updateProfileM.gender,
        "SchoolYear": updateProfileM.schoolYear,
        "SchoolKey": updateProfileM.schoolKey,
      });
  ...
  Future<String> changePass(String token, ChangePasswordN changePasswordN) async {
    print('change pass token:' + token);
    try {
      final body = FormData.fromMap({
        "OldPassword": changePasswordN.oldPass,
        "NewPassword": changePasswordN.newPass
      });
  ...
  Future<String> forgetPass(forgetpassw forgetPass) async {
    try {
      final body = FormData.fromMap({
        "accountID": forgetPass.accountID,
        "numberID": forgetPass.numberID,
        "newPass": forgetPass.newPass
      });
